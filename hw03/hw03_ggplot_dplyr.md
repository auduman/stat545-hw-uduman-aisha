...
I think this is a much better way to visualize the minimum and maximum GDP per capita between each continent. I used the `labs` function we went over in class to label the y-axis to "GDP per capita", the x-axis to "Continent" and to make a title for the plot.
...
task2 <- gapminder %>%
  group_by(continent) %>% 
  summarize(mean_gdp = mean(gdpPercap), min_gdp = min(gdpPercap), max_gdp = max(gdpPercap), sd_gdp = sd(gdpPercap))
