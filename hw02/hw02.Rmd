...
I logged the x axis (GDP per capita) because is it almost exponential and was not visually informative left as is. 


```{r}
p2 <- Asia %>% 
    group_by(country) %>% 
    summarize(lifeExp=mean(lifeExp), gdpPercap=mean(gdpPercap))
p2
```
A plot of countries in Asia and their mean GDP per capitas:

```{r}
p3 <- ggplot(p2, aes(x=country, y=gdpPercap))
p3 + geom_point()
```

A plot of countries in Asia and their mean life expentancies:
```{r}
p4 <- ggplot(p2, aes(x=country, y=lifeExp))
p4 + geom_point()
```
...
This took me much longer than Homework 1, which is expected as it goes into concepts in much more detail. I found the class on piping went a bit over my head, so I had to do some readings and use some online resources to better understand the syntax. 

**Some problems I encountered**

- Colouring my density plot 
